  ...
  return (
    <div className="mb-4 sm:mb-6 flex flex-col gap-2 sm:gap-3 md:flex-row md:items-end md:justify-between">
      <div className="min-w-0 flex-1">
        <p className="text-xs sm:text-sm font-semibold uppercase tracking-[0.25em] sm:tracking-[0.3em] text-teal-600">{eyebrow}</p>
        <h2 className="mt-1 sm:mt-2 text-lg sm:text-2xl lg:text-3xl font-semibold text-slate-900">{title}</h2>
        <p className="mt-1 sm:mt-2 text-xs sm:text-sm lg:text-base leading-5 sm:leading-6 text-slate-600">{description}</p>
      </div>
      {actionHref && actionLabel ? (
        <Link href={actionHref} className="inline-flex shrink-0 items-center gap-1 sm:gap-2 text-xs sm:text-sm font-semibold text-slate-700 transition hover:text-teal-700 whitespace-nowrap mt-2 sm:mt-0">
          {actionLabel}
...
const response = await fetch(`${baseUrl}/api/products`, {
  ...
  const newArrivals = activeProducts.slice(0, 16);
  ...
  const categories = Array.from(
    new Map(
      activeProducts
        .map((product) => {
          const name = product.category || product.brand || 'Other';
          const slug = product.categorySlug || name.toLowerCase().replace(/\s+/g, '-');
  ...
  const showcaseProducts = buildShowcaseProducts(activeProducts);

  return (
    <main className="page-transition bg-slate-50/70">
      {/* Compact Intro Section - Minimal, directing users to products */}
      <section className="mx-auto max-w-7xl px-3 py-3 sm:px-4 sm:py-4 lg:px-8">
        <div className="rounded-lg sm:rounded-[20px] border border-slate-200 bg-white/70 p-3 sm:p-6 backdrop-blur">
          <div className="flex flex-col gap-2 sm:gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="min-w-0 flex-1">
              <p className="text-xs sm:text-sm font-semibold uppercase tracking-[0.25em] sm:tracking-[0.3em] text-teal-600">Welcome to BIGZICO</p>
              <h1 className="mt-1 sm:mt-2 text-base sm:text-xl lg:text-2xl font-bold text-slate-900">
                Premium appliances, trusted nationwide
              </h1>
              <p className="mt-1 text-xs sm:text-sm text-slate-600">Authentic products with warranty support and expert installation</p>
            </div>
            <Link href="/products" className="inline-flex shrink-0 rounded-full bg-slate-900 px-4 sm:px-5 py-2 sm:py-3 text-xs sm:text-sm font-semibold text-white transition hover:bg-slate-700 whitespace-nowrap mt-2 sm:mt-0">
              Browse all products
            </Link>
          </div>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-3 py-3 sm:px-4 sm:py-4 lg:px-8">
        <div className="flex flex-wrap gap-1 sm:gap-2 rounded-full border border-slate-200 bg-white/80 p-1 sm:p-2 shadow-sm backdrop-blur">
          {categoryPills.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="rounded-full px-2 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm font-semibold text-slate-600 transition hover:bg-slate-900 hover:text-white whitespace-nowrap"
            >
              {item.label}
            </Link>
          ))}
      ...
      <section className="mx-auto max-w-7xl px-3 py-6 sm:px-4 sm:py-8 lg:px-8 lg:py-12">
        <SectionHeading
      ...
      <section className="mx-auto max-w-7xl px-3 py-6 sm:px-4 sm:py-8 lg:px-8 lg:py-12">
        ...
        <div className="grid gap-3 sm:gap-4 lg:grid-cols-2">
          {categorySpotlights.map((item) => (
            <div key={item.slug} className="rounded-lg sm:rounded-[24px] border border-slate-200 bg-white p-3 sm:p-5 shadow-sm">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between sm:gap-3">
                <div className="min-w-0 flex-1">
                  <p className="text-xs sm:text-sm font-semibold uppercase tracking-[0.25em] sm:tracking-[0.3em] text-teal-600">{item.name}</p>
                  <h3 className="mt-1 sm:mt-2 text-base sm:text-xl font-semibold text-slate-900 break-words">Popular picks for {item.name.toLowerCase()}</h3>
                </div>
                <Link href={`/categories/${item.slug}`} className="shrink-0 text-xs sm:text-sm font-semibold text-slate-700 transition hover:text-teal-700 whitespace-nowrap">
                  View all
                </Link>
              </div>
      ...
      <section className="mx-auto max-w-7xl px-3 py-6 sm:px-4 sm:py-8 lg:px-8 lg:py-12">
        ...
        <div className="space-y-6 sm:space-y-10">
          {groupedProducts.map((group) => (
            <section key={group.slug} className="rounded-lg sm:rounded-[28px] border border-slate-200 bg-white p-3 sm:p-6 shadow-sm">
              <div className="mb-4 sm:mb-6 flex flex-col gap-2 sm:gap-4 sm:flex-row sm:items-end sm:justify-between">
                <div className="min-w-0">
                  <p className="text-xs sm:text-sm font-semibold uppercase tracking-[0.25em] sm:tracking-[0.3em] text-teal-600">{group.name}</p>
                  <h3 className="mt-1 sm:mt-2 text-base sm:text-xl font-semibold text-slate-900">{group.items.length} products available</h3>
                </div>
                <Link href={`/categories/${group.slug}`} className="inline-flex shrink-0 items-center gap-1 sm:gap-2 text-xs sm:text-sm font-semibold text-slate-700 transition hover:text-teal-700 whitespace-nowrap">
                  Browse {group.name}
                  <span aria-hidden>→</span>
                </Link>
              </div>
                ...
                {group.items.slice(0, 4).map((product) => (
                  <Link key={product.id} href={`/products/${product.slug}`} className="overflow-hidden rounded-lg sm:rounded-[20px] border border-slate-200 bg-slate-50 transition hover:-translate-y-1 hover:shadow-md">
                    ...
                    <div className="p-2 sm:p-4">
                      ...
                      <p className="mt-1 sm:mt-3 text-sm sm:text-base font-semibold text-teal-700">₦{product.price.toLocaleString()}</p>
                    </div>
                  </Link>
                ))}
              </div>
      ...
      <section className="mx-auto max-w-7xl px-3 py-6 sm:px-4 sm:py-8 lg:px-8 lg:py-12">
        <div className="rounded-lg sm:rounded-[30px] border border-slate-200 bg-white p-4 sm:p-6 lg:p-10 shadow-sm">
          <SectionHeading
          ...
          <div className="grid gap-3 sm:gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
            {whyChooseItems.map((item) => (
              <div key={item.title} className="rounded-lg sm:rounded-[22px] border border-slate-200 bg-slate-50 p-3 sm:p-5 transition hover:-translate-y-1 hover:shadow-md">
                <div className="flex h-10 w-10 sm:h-11 sm:w-11 items-center justify-center rounded-full bg-teal-600/10 text-lg font-semibold text-teal-700">
                  {item.icon}
                </div>
                <h3 className="mt-2 sm:mt-4 text-base sm:text-lg font-semibold text-slate-900">{item.title}</h3>
      ...
      <section className="mx-auto max-w-7xl px-3 py-6 sm:px-4 sm:py-8 lg:px-8 lg:py-12">
        <div className="rounded-lg sm:rounded-[30px] border border-slate-200 bg-slate-950 p-4 sm:p-6 lg:p-10 text-white shadow-sm">
          <div className="flex flex-col gap-4 sm:gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div className="min-w-0 flex-1">
              <p className="text-xs sm:text-sm font-semibold uppercase tracking-[0.25em] sm:tracking-[0.3em] text-teal-300">Professional installation & repair services</p>
              <h2 className="mt-2 text-xl sm:text-2xl lg:text-3xl font-semibold">Expert help that keeps your appliances running beautifully.</h2>
              <p className="mt-2 sm:mt-3 text-xs sm:text-sm lg:text-base leading-5 sm:leading-6 text-slate-300">
                Book a qualified technician for installation, repairs, and ongoing support across major Nigerian locations.
              </p>
            </div>
            <Link href="/services" className="inline-flex shrink-0 rounded-full bg-white px-4 sm:px-5 py-2 sm:py-3 text-xs sm:text-sm font-semibold text-slate-900 transition hover:bg-slate-100 whitespace-nowrap mt-2 sm:mt-0">
              Book a technician
            </Link>
          </div>
