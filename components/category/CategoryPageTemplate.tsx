...
import {
  refrigeratorsProducts,
  freezersProducts,
  airConditionersProducts,
...
// Map of category slugs to their product datasets
const categoryProductsMap: Record<string, Product[]> = {
  refrigerators: refrigeratorsProducts,
  freezers: freezersProducts,
  'air-conditioners': airConditionersProducts,
  'washing-machines': washingMachinesProducts,
  televisions: televisionsProducts,
  generators: generatorsProducts,
  microwaves: microwavesProducts,
  'kitchen-appliances': kitchenAppliancesProducts,
  smartphones: smartphonesProducts,
  laptops: laptopsProducts,
};

export function CategoryPageTemplate({ category, content }: { category: { slug: string; name: string; description: string; image: string }; content: CategoryPageContent }) {
  const [activeFilter, setActiveFilter] = useState('All');
  const settings = getCategorySettings(category.slug);

  const visibleProducts = useMemo(() => {
    const categoryProducts = categoryProductsMap[category.slug] || [];
    if (activeFilter === 'All') return categoryProducts;
    return categoryProducts.filter((product) => product.subcategory?.toLowerCase() === activeFilter.toLowerCase() || product.tags.some((tag) => tag.toLowerCase() === activeFilter.toLowerCase()));
  ...
  return (
    <main className="page-transition bg-slate-50 text-slate-900">
      <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
        {/* Header Section */}
        {settings.showHeader && (
          <header className="rounded-[24px] border border-slate-200 bg-white/90 px-4 py-4 shadow-sm backdrop-blur sm:px-6">
            <div className="flex items-center justify-between gap-4">
              <Link href="/" className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold uppercase tracking-[0.3em] text-teal-600">BIGZICO</p>
                  <p className="text-base font-bold text-slate-900">Premium Appliances</p>
                </div>
              </Link>
        ...
        {settings.showFilters && (
          <section className="mx-auto max-w-7xl px-0 py-8 sm:py-10">
            <div className="mb-6 flex flex-col gap-4 rounded-[24px] border border-slate-200 bg-white p-5 shadow-sm lg:flex-row lg:items-center lg:justify-between">
              <div>
                <p className="text-sm font-semibold uppercase tracking-[0.3em] text-teal-600">Quick filters</p>
                <h2 className="mt-2 font-display text-2xl font-bold text-slate-900">Find the right fit</h2>
              </div>
        ...
        {settings.showLimitedOffer && (
          <section className="mx-auto max-w-7xl px-0 py-6 sm:py-8">
            <div className="rounded-[28px] border border-slate-200 bg-gradient-to-r from-slate-900 to-teal-800 p-8 text-white shadow-lg">
              <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
                <div>
                  <p className="text-sm font-semibold uppercase tracking-[0.3em] text-teal-300">Limited offer</p>
                  <h2 className="mt-2 font-display text-3xl font-bold">Save on Selected Home Appliances</h2>
                  <p className="mt-3 max-w-2xl text-sm text-slate-200 sm:text-base">Enjoy exclusive savings on premium products while stocks last.</p>
                </div>
                <Link href="/deals" className="inline-flex rounded-full bg-white px-5 py-3 font-semibold text-slate-900">Shop Deals</Link>
              </div>
            </div>
          </section>
        )}
            ...
            <div className="grid gap-4 lg:grid-cols-2">
              {settings.showTrendingNow && (
                <div className="rounded-[24px] border border-slate-200 bg-white p-6 shadow-sm">
                  <p className="text-sm font-semibold uppercase tracking-[0.3em] text-teal-600">Trending now</p>
                  <h2 className="mt-2 font-display text-2xl font-bold text-slate-900">Trending Now</h2>
                  <div className="mt-6 space-y-3">
                    {trendingProducts.map((product) => (
                      <div key={product.id} className="flex items-center justify-between rounded-2xl bg-slate-50 p-3">
                        <div>
                          <p className="font-semibold text-slate-900">{product.name}</p>
                          <p className="mt-1 text-sm text-slate-600">₦{product.price.toLocaleString()}</p>
                        </div>
                        <div className="rounded-full bg-amber-500 px-3 py-1 text-xs font-semibold text-white">Trending</div>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {settings.showBrowseCategories && (
                <div className="rounded-[24px] border border-slate-200 bg-white p-6 shadow-sm">
                  <p className="text-sm font-semibold uppercase tracking-[0.3em] text-teal-600">Browse by category</p>
                  <h2 className="mt-2 font-display text-2xl font-bold text-slate-900">Explore more categories</h2>
                  <div className="mt-6 grid gap-3 sm:grid-cols-2">
                    {categories.slice(0, 8).map((item) => (
                      <Link key={item.slug} href={`/categories/${item.slug}`} className="rounded-2xl border border-slate-200 bg-slate-50 p-4 transition hover:border-teal-500">
                        <p className="font-semibold text-slate-900">{item.name}</p>
                        <p className="mt-2 text-sm text-slate-600">Explore now</p>
                      </Link>
                    ))}
                  </div>
                </div>
        ...
        {settings.showTrustedBrands && (
          <section className="mx-auto max-w-7xl px-0 py-6 sm:py-8">
            <div className="rounded-[24px] border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="mt-6 grid grid-cols-2 gap-3 text-center text-sm font-semibold text-slate-700 sm:grid-cols-3 lg:grid-cols-5">
                {['LG', 'Samsung', 'Midea', 'Hisense', 'Thermocool', 'Daikin', 'Bosch', 'Sony', 'Philips', 'Anker'].map((brand) => <div key={brand} className="rounded-2xl bg-slate-50 px-4 py-3">{brand}</div>)}
              </div>
            </div>
          </section>
        )}
        ...
        {settings.showMainCollection && (
          <section className="mx-auto max-w-7xl px-0 py-6 sm:py-8">
            <div className="rounded-[24px] border border-slate-200 bg-white p-6 shadow-sm">
              <p className="text-sm font-semibold uppercase tracking-[0.3em] text-teal-600">Main product collection</p>
        ...
        <footer className="border-t border-slate-200 bg-white/90">
          <div className="mx-auto grid max-w-7xl gap-8 px-4 py-10 md:grid-cols-4 sm:px-6 lg:px-8">
            ...
            <div>
              ...
              <ul className="mt-3 space-y-2 text-sm text-slate-600">
                {categories.slice(0, 6).map((item) => <li key={item.slug}><Link href={`/categories/${item.slug}`}>{item.name}</Link></li>)}
              </ul>
            </div>
            <div>
