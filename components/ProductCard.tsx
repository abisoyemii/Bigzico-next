...
export function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <article className="group overflow-hidden rounded-[18px] border border-slate-200 bg-white shadow-[0_12px_35px_-20px_rgba(15,23,42,0.25)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_18px_45px_-20px_rgba(15,23,42,0.35)]">
      <Link href={`/products/${product.slug}`} className="block">
        ...
        <div className="p-3">
          <p className="text-[10px] font-semibold uppercase tracking-[0.25em] text-teal-600">{product.category}</p>
          <h3 className="mt-2 min-h-10 text-sm font-semibold text-slate-900 line-clamp-2">{product.name}</h3>
          ...
          <div className="mt-2 flex items-end gap-2">
            ...
            {product.compareAtPrice ? <span className="text-xs text-slate-400 line-through">₦{product.compareAtPrice.toLocaleString()}</span> : null}
          </div>
          <p className="mt-2 text-xs text-slate-500">{product.stock > 0 ? `${product.stock} in stock` : 'Out of stock'}</p>
        </div>
      </Link>
      <div className="flex items-center gap-2 border-t border-slate-200 p-3">
        <button onClick={() => addToCart(product)} className="flex-1 rounded-full bg-slate-900 px-2 py-2 text-[10px] font-semibold text-white transition hover:bg-slate-700 sm:text-xs sm:px-3">
          Add to Cart
        </button>
      </div>
    </article>
